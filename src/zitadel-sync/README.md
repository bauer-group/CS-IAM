...
```bash
docker compose run --rm zitadel-sync import-users --test-one   # verify userId=OID is accepted
docker compose run --rm zitadel-sync import-users              # bulk import (subject-preservation)
