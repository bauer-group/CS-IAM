...
- **Dev:** a Traefik `proxy` mirrors prod — one HTTPS origin, TLS (self-signed)
  ...
  else → core). Add `127.0.0.1 iam.example.test` to your hosts file and browse
  **`https://iam.example.test:8080`** (Console, Login, issuer, APIs — all one origin);
